...
export const AddProjectModal = observer(function AddProjectModal({
  ...
  const reportCreationError = (error: ProjectCreationError) => {
  ...
  const finishWith = (projectId: string) => {
    setCloseGuard(false);
    setSubmitState('idle');
    onClose();
  ...
  const startCreate = async (): Promise<string | null> => {
    ...
    const projectType: ProjectType = { type: 'local' as const };
    const data: ProjectCreationModeData = isRemoteRun
      ? {
          mode: 'pick',
          ...
          remoteConfig: { sshHost: runHost, remoteRepoDir: trimmedRemoteDir },
        }
      : {
          mode: 'pick',
          name: pickState.name,
  ...
  const handleSubmit = async () => {
    ...
    setCloseGuard(true);
    try {
      const projectId = await startCreate();
      if (projectId) {
        await registerSelectedSubagents();
        finishWith(projectId);
      } else {
        setCloseGuard(false);
        setSubmitState('idle');
      }
