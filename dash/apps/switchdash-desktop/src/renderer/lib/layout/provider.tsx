...
import {
  clearTelemetrySessionScope,
  setTelemetrySessionScope,
} from '@renderer/utils/telemetry-scope';
...
function syncTelemetryScope(currentViewId: ViewId, viewParamsStore: ViewParamsStore): void {
  ...
  const sessionParams = viewParamsStore.session;
  if (
    sessionParams &&
    typeof sessionParams.projectId === 'string' &&
    typeof sessionParams.sessionId === 'string'
  ) {
    setTelemetrySessionScope({
