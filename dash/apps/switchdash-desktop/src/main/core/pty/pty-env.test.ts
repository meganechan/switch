...
describe('buildAgentEnv provider env forwarding', () => {
  ...
  it('adds provider vars while keeping hook variables authoritative', async () => {
    ...
    const env = buildAgentEnv({
      ...
      providerVars: {
        ...
        EDITOR: 'vim',
        SWITCHDASH_HOOK_PORT: '9999',
        SWITCHDASH_PTY_ID: 'wrong',
        SWITCHDASH_HOOK_TOKEN: 'wrong-token',
      },
    });
  ...
  it('marks hook-managed sessions so the in-session Switch channel cedes polling', async () => {
    const { buildAgentEnv } = await loadPtyEnv();
    ...
    const unmanaged = buildAgentEnv({ agentApiVars: false });
    expect(unmanaged.SWITCH_CHANNEL_DISABLE_POLL).toBeUndefined();
  });
});
