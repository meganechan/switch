...
export class SshFileSystem implements FileSystemProvider {
  ...
  /**
   * Close the cached SFTP channel, if one was opened. SFTP channels — unlike
   * short-lived `exec` channels — do not self-close when an operation finishes,
   * and they are not gated by the proxy's exec-slot semaphore. A caller that
   * spins up a throwaway SshFileSystem for a one-shot read/write must call this
   * (in a `finally`) or it leaks a session channel; enough leaks exhaust the
   * server's MaxSessions and every later channel open fails with "open failed".
   */
  close(): void {
