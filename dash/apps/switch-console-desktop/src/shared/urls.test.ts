...
describe('switchConsoleReleaseTag', () => {
  it('namespaces the tag with the switchdash prefix', () => {
    expect(switchConsoleReleaseTag('0.18.2')).toBe('switchdash-v0.18.2');
  });

  it('does not double the v when the version already carries one', () => {
    expect(switchConsoleReleaseTag('v0.18.2')).toBe('switchdash-v0.18.2');
  });

  it('keeps a prerelease identifier intact', () => {
    expect(switchConsoleReleaseTag('0.19.0-canary.4')).toBe('switchdash-v0.19.0-canary.4');
  });
});
