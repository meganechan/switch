...
def _get_protocol() -> ProtocolService:
    assert _protocol is not None, "MCP protocol not initialized"
    return _protocol
...
async def _require_connected_room(ctx: Context) -> str:
    ...
    async with protocol.session_factory() as db:
        result = await protocol.agent_session_store.get_connected_room(
            db, transport_session_id
        )
    if result is None:
        raise ValueError("Not connected to a room. Call connect_to_room first.")
    _, room_id = result
...
@mcp.tool
async def list_references(ctx: Context) -> dict[str, Any]:
...
@mcp.tool
async def list_linked_rooms(ctx: Context) -> list[dict[str, Any]]:
...
@mcp.tool
async def load_internal_documents(ids: list[str], ctx: Context) -> list[dict[str, Any]]:
...
@mcp.tool
async def delete_room_document(document_id: str, ctx: Context) -> dict[str, str]:
...
@mcp.tool
async def list_participants(ctx: Context) -> list[dict[str, Any]]:
...
async def post_message(
    body: str, ctx: Context, thread_id: str | None = None
) -> dict[str, str]:
...
async def list_tasks(
    ...
    """
    agent_id = _get_agent_id()
    room_id = await _require_connected_room(ctx)  # type: ignore[arg-type]
...
@mcp.tool
async def list_roles(ctx: Context) -> list[dict[str, Any]]:
    ...
    agent_id = _get_agent_id()
    room_id = await _require_connected_room(ctx)
    return await _get_protocol().list_room_roles(agent_id, room_id)
...
async def get_role_detail(room_id: str, role_name: str) -> dict[str, Any]:
    ...
    """
    agent_id = _get_agent_id()
    return await _get_protocol().get_room_role(agent_id, room_id, role_name)


@mcp.tool
async def define_role(
    ctx: Context, name: str, instructions: str, exclusive: bool = False
) -> dict[str, Any]:
    ...
    agent_id = _get_agent_id()
    room_id = await _require_connected_room(ctx)
    await _get_protocol().define_room_role(
...
async def edit_role(
    ...
    agent_id = _get_agent_id()
    room_id = await _require_connected_room(ctx)
    await _get_protocol().edit_room_role(
        agent_id, room_id, name, instructions, exclusive
    )
...
@mcp.tool
async def delete_role(ctx: Context, name: str) -> dict[str, Any]:
    ...
    agent_id = _get_agent_id()
    room_id = await _require_connected_room(ctx)
    await _get_protocol().delete_room_role(agent_id, room_id, name)
    return {"status": "deleted", "name": name}


@mcp.tool
async def assume_role(ctx: Context, role: str) -> dict[str, Any]:
...
@mcp.tool
async def release_role(ctx: Context) -> dict[str, Any]:
    """Release the role you currently hold, freeing it for others. Idempotent."""
    agent_id = _get_agent_id()
    await _require_connected_room(ctx)
    await _get_protocol().release_room_role(agent_id)
