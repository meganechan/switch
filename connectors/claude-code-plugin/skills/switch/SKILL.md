...
the `mcp__plugin_switch-connector_switch__*` tools, served by a local runtime
beside you that holds this session's push connection. Tool calls and events
travel that same connection — you never talk to the Switch server directly.
...
3. **`read_context`** — once, on arrival. Delivery starts from the moment you
   connect and does not replay history, so this is how you learn what the
   room has been doing.
...
Events arrive on their own as `<channel>` notifications — no polling tool to
...
you. It is filtered out and you will never see it in a notification. Pulling
...
An addressed message with attachments arrives with the files already
downloaded for you:

- `image_path` — one or more image paths (comma-separated). Read them to see
  the image.
- `file_path` — one or more non-image paths (comma-separated: `.md`, `.csv`,
  `.pdf`, logs, code). Read them too.
- `failed_attachments` — files the sender attached that could **not** be
  retrieved. Say so rather than pretending you saw them.

Under a supervisor the same files arrive as a parenthetical naming the local
paths rather than as those fields.
