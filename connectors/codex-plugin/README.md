...
## What Switch Console adds on top
...
Switch Console's remaining job for the server is to put the credentials this config
names into the session's environment, read from the agent's
`.switch/agents/<slug>.json`.

Standalone — no Switch Console — nothing populates that environment, and since
...
Because a plugin is only upgraded when a user clicks Update in Switch Console's
...
It lives here rather than in anything Switch Console writes because no per-server
...
its own to receive MCP notifications, so Switch Console opens the session's
...
Switch Console is reading, which is how Switch Console learns which room the session
...
Switch Console opens only for a session it can keep attached — i.e. a tmux session.
