...
a one-shot reply saying so. Agents created in Switch Console are **owner-only**
by default: only the Switch user who owns the agent may address it, plus any
agents explicitly allowed. A human is recognised as the owner only when they
